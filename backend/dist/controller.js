...
const client_1 = require("@prisma/client"); // Cliente do Prisma para interação com o banco de dados.
const http_result_1 = require("./models/http-result"); // Modelo para padronizar as respostas HTTP (sucesso ou erro).
const utils_1 = require("./utils/utils"); // Funções utilitárias para validações (e.g., validação de email, senha, nome).
const dotenv_1 = __importDefault(require("dotenv")); // Carrega variáveis de ambiente do arquivo .env.
const bcryptjs_1 = __importDefault(require("bcryptjs")); // Biblioteca para criptografar e comparar senhas.
const jsonwebtoken_1 = __importDefault(require("jsonwebtoken")); // Biblioteca para geração e verificação de tokens JWT.
dotenv_1.default.config(); // Carrega as variáveis de ambiente definidas no arquivo .env.
const SECRET_KEY = process.env.SECRET_KEY; // Chave secreta utilizada para assinatura dos tokens JWT.
const prisma = new client_1.PrismaClient(); // Cria uma instância do PrismaClient para comunicação com o banco de dados.
